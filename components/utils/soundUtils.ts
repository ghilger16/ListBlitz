...
// Setter to update mute state; pauses or resumes the current sound when muting/unmuting
...
export const playSound = async (soundFile: any, loop: boolean = false) => {
  try {
    ...
    // Immediately apply mute if needed
    if (isMuted) {
      try {
        await soundInstance.setIsMutedAsync(true);
      } catch (error) {
        console.error("Error muting sound after start:", error);
      }
    }
